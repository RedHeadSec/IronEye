...
pub fn get_machine_account_quota(config: &mut LdapConfig) -> Result<(), Box<dyn Error>> {
    ...
    if let Some(entry) = entries.first() {
        ...
        for entry in entries {
            ...
            if let Some(name) = group_entry.attrs.get("sAMAccountName").and_then(|v| v.first()) {
                // Skip if it's a default group
                if DEFAULT_GROUPS.iter().any(|&default| name.eq_ignore_ascii_case(default)) {
                    continue;
                }
                ...
                if let Some(dn) = group_entry.attrs.get("distinguishedName").and_then(|v| v.first()) {
                    if dn.to_lowercase().contains("built-in") || 
                       ...
                       (dn.to_lowercase().contains("cn=users") && !dn.to_lowercase().contains("ou=")) {
                        continue;
                    }
                }
        ...
        match quota {
            ...
            10 => {
                println!("- Domain Users can create up to 10 computer accounts (Default Configuration)");
