    ...
    for entry in entries {
        ...
        if let Some(lockout_duration) = entry.attrs.get("lockoutDuration") {
            if let Ok(duration) = lockout_duration[0].parse::<i64>() {
                println!("Account Lockout Duration: {}", format_time_interval(duration));
            }
        }

        if let Some(lockout_window) = entry.attrs.get("lockOutObservationWindow") {
            if let Ok(window) = lockout_window[0].parse::<i64>() {
                println!("Lockout Observation Window: {}", format_time_interval(window));
            }
        }
    }
...
fn query_password_policy(ldap: &mut LdapConn, search_base: &str) -> Result<Vec<SearchEntry>, Box<dyn Error>> {
