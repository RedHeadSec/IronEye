...
impl HistoryManager {
    ...
    pub fn get_recent(&self, module: &str, limit: usize) -> Result<Vec<String>> {
        ...
        let commands = stmt.query_map((module, limit), |row| row.get(0))?
    ...
    pub fn clear_module(&self, module: &str) -> Result<usize> {
        self.conn.execute(
            "DELETE FROM history WHERE module = ?1",
            [module],
        )
    }
    ...
    pub fn export_to_file(&self, filepath: &str) -> Result<usize> {
        ...
        let mut stmt = self.conn.prepare(
            "SELECT module, command, timestamp FROM history ORDER BY timestamp DESC"
        )?;
...
impl HistoryEditor {
    pub fn new(module: &str) -> Result<Self> {
        ...
        // Load recent history into readline (reversed so oldest is first in readline history)
