...
pub fn del_user_from_group(
    ...
    println!("[*] Removing \"{}\" from \"{}\"", user, group);

    let mut member_set = HashSet::new();
    member_set.insert(user_dn.as_str());

    match ldap.modify(&group_dn, vec![Mod::Delete("member", member_set)]) {
        Ok(result) => match result.success() {
            ...
            Err(e) => {
                ...
                } else if error_string.contains("noSuchAttribute") || error_string.contains("16") {
                    eprintln!(
