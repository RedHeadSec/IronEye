...
fn parse_soa_serial(response: &[u8]) -> Result<u32, Box<dyn std::error::Error>> {
    ...
    if record_type != 6 {
        return Err(format!("Response is not an SOA record (got type {}, expected 6 SOA)", record_type).into());
    }
...
pub fn get_next_serial(
    dc_ip: &str,
    zone: &str,
) -> Result<u32, Box<dyn std::error::Error>> {
    debug_log(2, format!("Querying SOA via UDP: {}:{} for zone {}", dc_ip, 53, zone));
    ...
    debug_log(3, format!("Sending DNS query packet ({} bytes)", query.len()));
    ...
    debug_log(2, format!("Current SOA serial: {}, returning: {}", serial, serial + 1));
    Ok(serial + 1)
}


#[allow(dead_code)]
pub fn get_next_serial_tcp(
    dc_ip: &str,
    zone: &str,
) -> Result<u32, Box<dyn std::error::Error>> {
    debug_log(2, format!("Querying SOA via TCP: {}:{} for zone {}", dc_ip, 53, zone));
    ...
    debug_log(3, format!("Sending DNS query via TCP ({} bytes)", tcp_query.len()));
...
pub fn get_next_serial_with_fallback(
    ...
    debug_log(1, format!("Attempting to get SOA serial for zone: {}", zone));
