...
fn handle_query_zones(
    ...
    let domain_zones = retry_with_reconnect!(ldap, ldap_config, zones::query_dns_zones(ldap, search_base, false, false))?;
    ...
    println!("\n=== Forest DNS Zones ===");
    let forest_zones = retry_with_reconnect!(ldap, ldap_config, zones::query_dns_zones(ldap, search_base, true, false))?;
...
fn query_dns_record(
    ...
    debug_log(2, format!("Starting record query for: {}.{}", record_name, zone));
    ...
    let results = match results {
        ...
        None => {
            if let Some(target) = referral_target {
                ...
                println!("    Example: ironeye connect -u user -p pass -d {} -i <other-dc-ip>", target);
                return Ok(());
    ...
    for entry in results {
        ...
        let name = search_entry.attrs.get("name")
            .and_then(|v| v.first())
            .map(|s| s.clone())
            .unwrap_or_else(|| record_name.to_string());
        
        
        let is_tombstoned = search_entry.attrs.get("dNSTombstoned")
            .and_then(|v| v.first())
        ...
        if let Some(dns_records) = search_entry.bin_attrs.get("dnsRecord") {
            ...
            for (idx, record_data) in dns_records.iter().enumerate() {
                ...
                match structures::DnsRecord::from_bytes(record_data) {
                    Ok(record) => {
                        debug_log(3, format!("Parsed record: type={} serial={}", record.record_type, record.serial));
...
fn print_dns_record(record: &structures::DnsRecord) {
    println!("Type: {} ({})", record.record_type, structures::get_record_type_name(record.record_type));
    ...
    match record.record_type {
        ...
        _ => {
            println!("Data: {} bytes (raw data not displayed for this record type)", record.data.len());
        }
    }
}
...
fn handle_add_record(
    ...
    let ttl = if ttl_input.is_empty() {
        ...
        match ttl_input.parse::<u32>() {
            Ok(t) => t,
            Err(_) => {
                debug_log(2, format!("Invalid TTL input: {}, using default", ttl_input));
    ...
    debug_log(1, format!("Adding A record: {}.{} -> {} (TTL: {})", record_name, zone, ip_address, ttl));
    println!("[*] Adding A record: {}.{} -> {}", record_name, zone, ip_address);
    
    add_a_record(ldap, search_base, &ldap_config.dc_ip, &record_name, &ip_address, &zone, ttl)?;
...
fn add_a_record(
    ...
    debug_log(1, format!("Starting add_a_record: {}.{} -> {}", record_name, zone, ip_address));
    ...
    let zone_dn = format!("DC={},CN=MicrosoftDNS,DC=DomainDnsZones,{}", zone, search_base);
    ...
    debug_log(3, format!("Checking if record exists with filter: {}", filter));
    ...
    debug_log(3, format!("DNS record binary size: {} bytes", record_bytes.len()));
    ...
    match ldap.add(&record_dn, attrs) {
        Ok(result) => {
            match result.success() {
                Ok(_) => {
                    println!("[+] Successfully created DNS node: {}.{}", record_name, zone);
                    ...
                    match ldap.modify(&record_dn, vec![Mod::Add(attr_name, dns_record_set)]) {
                        Ok(mod_result) => {
                            match mod_result.success() {
                                Ok(_) => {
                                    println!("[+] Successfully added A record: {}.{} -> {}", record_name, zone, ip_address);
...
fn handle_modify_record(
    ...
    debug_log(1, format!("Modify record requested: {}.{} -> {}", record_name, zone, new_ip));
    println!("[*] Modifying A record: {}.{} -> {}", record_name, zone, new_ip);
    
    modify_a_record(ldap, search_base, &ldap_config.dc_ip, &record_name, &new_ip, &zone)?;
...
fn modify_a_record(
    ...
    debug_log(1, format!("Starting modify_a_record: {}.{} -> {}", record_name, zone, new_ip));
    ...
    debug_log(3, format!("Searching for existing record with filter: {}", filter));
    ...
    debug_log(2, format!("Found {} existing record(s)", existing_records.len()));
    ...
    for (idx, record_data) in existing_records.iter().enumerate() {
        match structures::DnsRecord::from_bytes(record_data) {
            Ok(record) => {
                debug_log(3, format!("Record {}: type={} serial={}", idx + 1, record.record_type, record.serial));
                if record.record_type == structures::record_types::A {
                    has_a_record = true;
                    println!("[*] Found existing A record: {}", structures::format_a_record(&record.data));
                }
    ...
    for record in parsed_records {
        if record.record_type == structures::record_types::A {
            ...
            debug_log(3, format!("Keeping record type: {}", kept_record.record_type));
        }
    }
    
    debug_log(2, format!("Prepared {} record(s) for replacement", new_records_bytes.len()));
    ...
    match ldap.modify(&record_dn, vec![Mod::Replace(attr_name, dns_record_set)]) {
        Ok(mod_result) => {
            match mod_result.success() {
                Ok(_) => {
                    println!("[+] Successfully modified A record: {}.{} -> {}", record_name, zone, new_ip);
...
fn tombstone_dns_record(
    ...
    debug_log(1, format!("Starting tombstone operation: {}.{}", record_name, zone));
    ...
    for (idx, record_data) in existing_records.iter().enumerate() {
        match structures::DnsRecord::from_bytes(record_data) {
            Ok(record) => {
                ...
                debug_log(3, format!("Record {}: type={} serial={}", idx + 1, type_name, record.serial));
                
                if record.record_type == structures::record_types::A {
                    println!("[*] Will tombstone A record: {}", structures::format_a_record(&record.data));
                }
    ...
    debug_log(2, format!("Prepared {} record(s) for tombstone", new_records_bytes.len()));
    ...
    match ldap.modify(&record_dn, modifications) {
        Ok(mod_result) => {
            match mod_result.success() {
                Ok(_) => {
                    println!("[+] Successfully tombstoned DNS record: {}.{}", record_name, zone);
...
fn handle_delete_record(
    ...
    debug_log(1, format!("Delete record requested: {}.{}", record_name, zone));
...
fn delete_dns_record(
    ...
    debug_log(1, format!("Attempting to delete DNS record: {}.{}", record_name, zone));
    ...
    if results.is_empty() {
        ...
        match ldap.search(&forest_zone_dn, Scope::OneLevel, &filter, vec!["distinguishedName"]) {
            Ok(search_result) => {
...
fn perform_delete(
    ...
    match ldap.delete(record_dn) {
        Ok(result) => {
            match result.success() {
                Ok(_) => {
                    println!("\n[+] Successfully deleted DNS record: {}.{}", record_name, zone);
                ...
                Err(e) => {
                    ...
                    } else if error_string.contains("notAllowedOnNonLeaf") || error_string.contains("66") {
