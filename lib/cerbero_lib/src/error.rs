...
use crate::windows::error::{WinApiError, WinError};
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            #[cfg(target_os = "windows")]
            Error::WinApiError(wae) =>  write!(f, "{}", wae)
        }
    }
}
...
#[cfg(target_os = "windows")]
impl From<WinApiError> for Error {
    fn from(wae: WinApiError) -> Self {
        return Self::WinApiError(wae);
    }
}
...
impl From<WinError> for Error {
    fn from(e: WinError) -> Self {
        match e {
            WinError::String(s) => Self::String(s),
            WinError::WinApiError(wae) => Self::WinApiError(wae)
        }
    }
}
